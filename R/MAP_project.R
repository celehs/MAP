fitproj_flexmix = function(tmpfm, note, family, tmpfm2, dat.tmp, sample_size) {
  ...
  fit_sample = sample(seq_len(nrow(m_data)), sample_size)
...
#                             family = family)
  ...
  list(m_data = rbind(m_fit, m_project),
       m_post = rbind(flexmix::posterior(fit_obj$tmpfit), m_post),
       cluster = c(fit_obj$tmpfit@cluster, clusters))
}
